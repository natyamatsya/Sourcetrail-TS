...
struct ConcurrentTursoWriter::Impl
{
	...
	void process(TsqConn* conn, const Batch& batch)
	{
		...
		for (const StorageError& e: batch.errors)
		{
			...
			if (r.created)
			{
				sql += "INSERT INTO element(id) VALUES(" + s(r.id) + ");";
				sql += "INSERT INTO error(id,message,fatal,indexed,translation_unit) VALUES(" + s(r.id) +
		...
		for (const StorageNode& n: batch.nodes)
		{
			...
			{
				sql += "INSERT INTO element(id) VALUES(" + s(r.id) + ");";
				sql += "INSERT INTO node(id,type,serialized_name) VALUES(" + s(r.id) + "," +
		...
		for (const StorageFile& f: batch.files)
		{
			...
			sql += "INSERT INTO file(id,path,language,modification_time,indexed,complete,line_count) VALUES(" +
		...
		for (const StorageSymbol& sym: batch.symbols)
		{
			...
			{
				sql += "INSERT INTO symbol(id,definition_kind) VALUES(" + s(it->second) + "," +
		...
		for (const StorageEdge& e: batch.edges)
		{
			...
			{
				sql += "INSERT INTO element(id) VALUES(" + s(r.id) + ");";
				sql += "INSERT INTO edge(id,type,source_node_id,target_node_id) VALUES(" + s(r.id) + "," +
		...
		for (const StorageLocalSymbol& ls: batch.localSymbols)
		{
			...
			{
				sql += "INSERT INTO element(id) VALUES(" + s(r.id) + ");";
				sql += "INSERT INTO local_symbol(id,name) VALUES(" + s(r.id) + ",'" + esc(ls.name) + "');";
			}
		}
		...
		for (const StorageSourceLocation& l: batch.sourceLocations)
		{
			...
			{
				sql += "INSERT INTO element(id) VALUES(" + s(r.id) + ");";
				sql += "INSERT INTO source_location(id,file_node_id,start_line,start_column,end_line,end_column,type) VALUES(" +
		...
		for (const StorageOccurrence& o: batch.occurrences)
		{
			...
			{
				sql += "INSERT INTO occurrence(element_id,source_location_id) VALUES(" + s(eIt->second) +
					"," + s(lIt->second) + ");";
			}
		}
		...
		for (const StorageComponentAccess& c: batch.componentAccesses)
		{
			...
			{
				sql += "INSERT INTO component_access(node_id,type) VALUES(" + s(it->second) + "," +
		...
		for (const StorageElementComponent& c: batch.elementComponents)
		{
			...
			if (r.created)
			{
				sql += "INSERT INTO element_component(id,element_id,type,data) VALUES(" + s(r.id) + "," +
		...
		if (tsq_conn_exec(conn, "BEGIN CONCURRENT") != TSQ_OK)
		{
			...
			LOG_ERROR(std::string("concurrent writer BEGIN failed: ") + tsq_last_error());
			return;
		}
		if (!sql.empty() && tsq_conn_exec(conn, sql.c_str()) != TSQ_OK)
		{
			...
			failed.fetch_add(1, std::memory_order_relaxed);
			LOG_ERROR(std::string("concurrent writer INSERT failed: ") + tsq_last_error());
			return;
		}
		// filecontent: bound inserts (arbitrary/large text) in the same transaction.
		if (!fileContents.empty())
		{
			TsqStmt* stmt = tsq_conn_prepare(conn, "INSERT INTO filecontent(id, content) VALUES(?, ?)");
			if (stmt != nullptr)
			{
		...
		{
			failed.fetch_add(1, std::memory_order_relaxed);
			LOG_ERROR(std::string("concurrent writer COMMIT failed: ") + tsq_last_error());
		}
	}
