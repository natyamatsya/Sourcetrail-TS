...
auto make_pointer(T &&value, std::enable_if_t<std::is_pointer<Pointer>::value && std::is_reference<Reference>::value, int> = 0)
...
auto make_pointer(T &&value,
				  std::enable_if_t<!std::is_pointer<Pointer>::value && !std::is_same<Pointer, void>::value && std::is_reference<Reference>::value, int> = 0)
{
...
using interoperable = std::integral_constant<bool, (std::is_convertible<T, U>::value || std::is_convertible<U, T>::value)>;
...
struct common_eq
{
	static constexpr auto call(T lhs, U rhs) { return static_cast<common_t<T, U>>(lhs).derived() == static_cast<common_t<T, U>>(rhs).derived(); }
};
template <typename T, typename U>
...
template <typename T, typename U>
constexpr auto common_diff(T lhs, U rhs) noexcept(noexcept(static_cast<common_t<T, U>>(lhs) - static_cast<common_t<T, U>>(rhs)))
...
BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_NAMESPACE_V1
{
	...
	template <typename Derived, typename IteratorConcept, typename ValueType, typename Reference = ValueType &, typename Pointer = ValueType *, typename DifferenceType = std::ptrdiff_t
#ifndef BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_DOXYGEN
			  ,
			  typename E = std::enable_if_t<std::is_class<Derived>::value && std::is_same<Derived, std::remove_cv_t<Derived>>::value>
#endif
			  >
	...
	struct ra_iter<Iterator, void_t<typename Iterator::iterator_concept>>
		: std::integral_constant<bool, std::is_base_of<std::random_access_iterator_tag, typename Iterator::iterator_concept>::value>
	{
	};
	...
	struct plus_eq<Iterator, DifferenceType, void_t<decltype(std::declval<Iterator &>() += std::declval<DifferenceType>())>> : std::true_type
	{
	};

	template <typename D, typename IteratorConcept, typename ValueType, typename Reference, typename Pointer, typename DifferenceType>
	void derived_iterator(const iterator_interface<D, IteratorConcept, ValueType, Reference, Pointer, DifferenceType> &);
	} // namespace v1_dtl

	template <typename Derived, typename IteratorConcept, typename ValueType, typename Reference, typename Pointer, typename DifferenceType
#ifndef BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_DOXYGEN
			  ,
			  typename E
#endif
	...
	public:
		...
		constexpr auto operator*() noexcept(noexcept(*access::base(std::declval<D &>()))) -> decltype(*access::base(std::declval<D &>()))
		{
			return *access::base(derived());
		}
		template <typename D = Derived>
		constexpr auto operator*() const noexcept(noexcept(*access::base(std::declval<const D &>()))) -> decltype(*access::base(std::declval<const D &>()))
		{
			return *access::base(derived());
		}
		...
		template <typename D = Derived>
		constexpr auto operator->() const noexcept(noexcept(detail::make_pointer<pointer, reference>(*std::declval<const D &>())))
		...
		template <typename D = Derived>
		constexpr auto operator[](difference_type i) const noexcept(noexcept(D(std::declval<const D &>()), std::declval<D &>() += i, *std::declval<D &>()))
		...
		template <typename D = Derived, typename Enable = std::enable_if_t<!v1_dtl::plus_eq<D, difference_type>::value>>
		...
		template <typename D = Derived>
		constexpr auto operator+(difference_type i) const noexcept(noexcept(D(std::declval<D &>()), std::declval<D &>() += i))
		...
		friend BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_HIDDEN_FRIEND_CONSTEXPR Derived operator+(difference_type i, Derived it) noexcept
		{
			return it + i;
		}
		...
		template <typename D = Derived>
		constexpr auto operator--() noexcept(noexcept(D(std::declval<D &>()), std::declval<D &>() += -difference_type(1)))
			-> decltype(std::declval<D &>() += -difference_type(1), std::declval<D &>())
		{
			derived() += -difference_type(1);
			return derived();
		...
		template <typename D = Derived>
		constexpr auto operator-(D other) const noexcept(noexcept(access::base(std::declval<const D &>()) - access::base(other)))
			-> decltype(access::base(std::declval<const D &>()) - access::base(other))
		{
			return access::base(derived()) - access::base(other);
		}

		friend BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_HIDDEN_FRIEND_CONSTEXPR Derived operator-(Derived it, difference_type i) noexcept
		{
			Derived retval = it;
			retval += -i;
			return retval;
	...
	template <typename IteratorInterface1, typename IteratorInterface2, typename Enable = std::enable_if_t<!v1_dtl::ra_iter<IteratorInterface1>::value>>
	...
	template <typename IteratorInterface1, typename IteratorInterface2, typename Enable = std::enable_if_t<v1_dtl::ra_iter<IteratorInterface1>::value>>
	constexpr auto operator==(IteratorInterface1 lhs, IteratorInterface2 rhs) noexcept(noexcept(detail::common_diff(lhs, rhs)))
	...
	constexpr auto operator!=(IteratorInterface1 lhs, IteratorInterface2 rhs) noexcept(noexcept(!(lhs == rhs)))->decltype(v1_dtl::derived_iterator(lhs), !(lhs == rhs))
	{
		return !(lhs == rhs);
	}
	...
	/** A template alias useful for defining proxy iterators.  \see
        `iterator_interface`. */
	template <typename Derived, typename IteratorConcept, typename ValueType, typename Reference = ValueType, typename DifferenceType = std::ptrdiff_t>
	using proxy_iterator_interface = iterator_interface<Derived, IteratorConcept, ValueType, Reference, proxy_arrow_result<Reference>, DifferenceType>;
}
} // namespace stl_interfaces
} // namespace beman::optional::detail
...
BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_NAMESPACE_V2
{
	...
	template <typename IteratorConcept, typename ReferenceType, typename IteratorCategory = decltype(v2_dtl::category_tag<IteratorConcept, ReferenceType>())>
...
BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_NAMESPACE_V3
{
	...
	/** A template alias useful for defining proxy iterators.  \see
        `iterator_interface`. */
	template <typename IteratorConcept, typename ValueType, typename Reference = ValueType, typename DifferenceType = std::ptrdiff_t>
	using proxy_iterator_interface = iterator_interface<IteratorConcept, ValueType, Reference, proxy_arrow_result<Reference>, DifferenceType>;
}
} // namespace stl_interfaces
} // namespace beman::optional::detail
...
/** `static_asserts` that the types of all typedefs in
    ...
    is not defined. */
#define BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_ITERATOR_TRAITS(iter, category, concept, value_type, reference, pointer, difference_type)

#else

#define BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_ITERATOR_CONCEPT_IMPL(type, concept_name) static_assert(concept_name<type>, "");
...
#define BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_ITERATOR_TRAITS_IMPL(iter, category, value_t, ref, ptr, diff_t) \
...
#define BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_ITERATOR_TRAITS(iter, category, concept, value_type, reference, pointer, difference_type) \
	BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_ITERATOR_TRAITS_IMPL(iter, category, value_type, reference, pointer, difference_type)
#endif
