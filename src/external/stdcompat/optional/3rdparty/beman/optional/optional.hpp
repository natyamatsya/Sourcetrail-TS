...
constexpr optional<std::decay_t<T>> make_optional(T &&) noexcept(std::is_nothrow_constructible_v<optional<std::decay_t<T>>, T>)
...
constexpr optional<T> make_optional(std::initializer_list<U> il, Args &&...args) noexcept(std::is_nothrow_constructible_v<T, std::initializer_list<U> &, Args...>)
...
concept enable_forward_value = !std::is_same_v<std::decay_t<U>, optional<T>> && !std::is_same_v<std::decay_t<U>, in_place_t> &&
...
template <class T, class U>
concept enable_assign_forward = !std::is_same_v<optional<T>, std::decay_t<U>> && !std::conjunction_v<std::is_scalar<T>, std::is_same<T, std::decay_t<U>>> &&
								std::is_constructible_v<T, U> && std::is_assignable_v<T &, U>;

template <class T, class U, class Other>
concept enable_assign_from_other = std::is_constructible_v<T, Other> && std::is_assignable_v<T &, Other> &&
...
public:
	...
	constexpr optional &operator=(const optional &rhs)
		requires std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T> && (!std::is_trivially_copy_assignable_v<T>);
	...
	constexpr optional &operator=(const optional &)
		requires std::is_copy_constructible_v<T> && std::is_copy_assignable_v<T> && std::is_trivially_copy_constructible_v<T> && std::is_trivially_copy_assignable_v<T>
	= default;
	...
	constexpr optional &operator=(optional &&rhs) noexcept(std::is_nothrow_move_constructible_v<T>)
		requires std::is_move_constructible_v<T> && std::is_move_assignable_v<T> && (!std::is_trivially_move_assignable_v<T>);

	/**
     * @brief   Move assigns the value from \p rhs if it has one.
     */
	constexpr optional &operator=(optional &&)
		requires std::is_move_constructible_v<T> && std::is_move_assignable_v<T> && std::is_trivially_move_constructible_v<T> && std::is_trivially_move_assignable_v<T>
	= default;
     ...
     */
	constexpr void swap(optional &rhs) noexcept(std::is_nothrow_move_constructible<T>::value && std::is_nothrow_swappable<T>::value);
     ...
     */
	template <class F>
		constexpr optional or_else(F &&f) && requires(std::invocable<F> &&std::move_constructible<T>);
...
private:
	...
	constexpr optional(detail::from_function_t, F &&f, Arg &&arg)
		: value_(std::invoke(std::forward<F>(f), std::forward<Arg>(arg)))
		, engaged_(true)
	{
	}
};
...
template <class T>
inline constexpr optional<T> &optional<T>::operator=(optional<T> &&rhs) noexcept(std::is_nothrow_move_constructible_v<T>)
	requires std::is_move_constructible_v<T> && std::is_move_assignable_v<T> && (!std::is_trivially_move_assignable_v<T>)
{
	if(!rhs.has_value())
		reset();
...
/// valueless.
template <class T>
inline constexpr void optional<T>::swap(optional<T> &rhs) noexcept(std::is_nothrow_move_constructible<T>::value && std::is_nothrow_swappable<T>::value)
{
...
constexpr optional<std::decay_t<T>> make_optional(T &&t) noexcept(std::is_nothrow_constructible_v<optional<std::decay_t<T>>, T>)
	requires std::is_constructible_v<std::decay_t<T>, T>
{
...
constexpr optional<T> make_optional(std::initializer_list<U> init_list,
...
public:
	...
	template <class U>
		requires(std::is_constructible_v<T &, U> && !(std::is_same_v<std::remove_cvref_t<U>, in_place_t>) &&
				 !(std::is_same_v<std::remove_cvref_t<U>, optional>) && !detail::reference_constructs_from_temporary_v<T &, U>)
     ...
     */
	template <class U>
		requires(std::is_constructible_v<T &, U &> && !std::is_same_v<std::remove_cv_t<T>, optional<U>> && !std::is_same_v<T &, U> &&
				 !detail::reference_constructs_from_temporary_v<T &, U &>)
	constexpr explicit(!std::is_convertible_v<U &, T &>) optional(optional<U> &rhs) noexcept(std::is_nothrow_constructible_v<T &, U &>);
     ...
     */
	template <class U>
		requires(std::is_constructible_v<T &, const U &> && !std::is_same_v<std::remove_cv_t<T>, optional<U>> && !std::is_same_v<T &, U> &&
				 !detail::reference_constructs_from_temporary_v<T &, const U &>)
	constexpr explicit(!std::is_convertible_v<const U &, T &>) optional(const optional<U> &rhs) noexcept(std::is_nothrow_constructible_v<T &, const U &>);
	...
	template <class U>
		requires(std::is_constructible_v<T &, U> && !std::is_same_v<std::remove_cv_t<T>, optional<U>> && !std::is_same_v<T &, U> &&
				 !detail::reference_constructs_from_temporary_v<T &, U>)
	constexpr explicit(!std::is_convertible_v<U, T &>) optional(optional<U> &&rhs) noexcept(noexcept(std::is_nothrow_constructible_v<T &, U>));
...
template <class U>
	requires(std::is_constructible_v<T &, const U> && !std::is_same_v<std::remove_cv_t<T>, optional<U>> && !std::is_same_v<T &, U> &&
			 !detail::reference_constructs_from_temporary_v<T &, const U>)
constexpr optional<T &>::optional(const optional<U> &&rhs) noexcept(noexcept(std::is_nothrow_constructible_v<T &, const U>))
{
	if(rhs.has_value())
	{
		convert_ref_init_val(*std::move(rhs));
