/*
 * Unaligned-safe replacement for flatcc's <flatcc/flatcc_accessors.h>.
 *
 * flatcc's stock scalar accessors read/write through `*(T *)p` casts. That is
 * fine in C on targets that tolerate unaligned access (x86, arm64), and both
 * the C++ core and the C runtime rely on it — real FlatBuffers place a u64
 * table field at only 4-byte alignment when the buffer as a whole is 4-aligned.
 *
 * When these accessors are @cImport'ed into Zig, translate-c lowers `*(T *)p`
 * to `@as([*c]T, @ptrCast(@alignCast(p))).*`, and @alignCast inserts a runtime
 * alignment check. Reading such a buffer then aborts with `panic: incorrect
 * alignment` the moment a field is not naturally aligned — which is routine.
 *
 * This header is @cInclude'd BEFORE any flatcc header and defines the
 * FLATCC_ACCESSORS include guard, so flatcc's own version is skipped. It
 * provides the identical macro surface but routes every scalar read and write
 * through memcpy, which translate-c lowers to a byte copy with no alignment
 * assumption. Endianness handling (the *_cast_* helpers) is unchanged.
 */
