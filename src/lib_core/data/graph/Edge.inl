// Inline member definitions for Edge.h (included -- once both Edge and Node are complete -- from
// Node.h). All members are inline because an out-of-line member of an exported class does not resolve
// for module importers. The bodies below dereference Node, hence the completeness requirement.

#pragma once

template <>
inline Edge::EdgeType intToEnum(int value)
{
	switch (value)
	{
...
inline Edge::Edge(Id id, EdgeType type, Node* from, Node* to)
...
inline Edge::Edge(const Edge& other, Node* from, Node* to)
	: Token(other), m_type(other.m_type), m_from(from), m_to(to)
{
	...
	{
		srctrl::log::error("Nodes are not plain copies.");
	}
}
...
inline Edge::EdgeType Edge::getType() const
{
	return m_type;
}

inline bool Edge::isType(TypeMask mask) const
{
...
inline std::string Edge::getName() const
{
	return getReadableTypeString() + ":" + getFrom()->getFullName() + "->" + getTo()->getFullName();
}

inline bool Edge::isNode() const
{
	return false;
}

inline bool Edge::isEdge() const
{
	return true;
}

inline std::string Edge::getUnderscoredTypeString(EdgeType type)
{
	return utility::replace(utility::replace(getReadableTypeString(type), "-", "_"), " ", "_");
}

inline std::string Edge::getReadableTypeString(EdgeType type)
{
	switch (type)
	{
	case EDGE_UNDEFINED:
...
inline Edge::EdgeType Edge::getTypeForReadableTypeString(const std::string& str)
{
...
inline std::string Edge::getReadableTypeString() const
{
	return getReadableTypeString(m_type);
}

inline std::string Edge::getAsString() const
{
...
inline std::ostream& operator<<(std::ostream& ostream, const Edge& edge)
{
